...
class B2CUser private constructor() {
        ...
        for (account in accounts) {
            if (policyName.equals(getB2CPolicyNameFromAccount(account), ignoreCase = true)) {
                val parameters = AcquireTokenSilentParameters.Builder()
                    .fromAuthority(getAuthorityFromPolicyName()).withScopes(scopes)
                    .forAccount(account).withCallback(callback).build()
    ...
    companion object {
        ...
        private fun getB2CPolicyNameFromAccount(account: IAccount): String? {
            return account.claims!!["tfp"] as String? ?: // Fallback to "acr" (for older policies)
